...
export async function salesAddDevice(
  customer_id: string,
): Promise<{ id: string } | { error: string }> {
  try {
    await assertSubtreeCustomer(customer_id);
  } catch (e) {
    return { error: e instanceof Error ? e.message : "권한 오류" };
  }
  const db = createAdminClient();
  ...
  revalidatePath(`/sales/customers/${customer_id}`);
  return { id: data.id as string };
}

export async function salesSaveDevice(
  customer_id: string,
  device_id: string,
...
export async function salesDeleteDevice(
  customer_id: string,
  device_id: string,
): Promise<{ ok: true } | { error: string }> {
  try {
...
export async function salesCreateDevicePhotoUrl(
  customer_id: string,
  device_id: string,
  filename: string,
): Promise<{ path: string; token: string } | { error: string }> {
...
export async function salesRecordDevicePhoto(
  customer_id: string,
  device_id: string,
  path: string,
): Promise<{ ok: true } | { error: string }> {
...
export async function salesDeleteDevicePhoto(
  customer_id: string,
  doc_key: string,
): Promise<{ ok: true } | { error: string }> {
  try {
